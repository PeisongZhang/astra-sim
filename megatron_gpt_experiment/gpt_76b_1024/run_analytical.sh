...
if [ ! -f "${WORKLOAD_DIR}/workload.0.et" ]; then
    echo "[ASTRA-sim] ERROR: workload.0.et not found in ${WORKLOAD_DIR}. Run dnn_workload/megatron_gpt_76b/megatron_gpt_76b.sh first." >&2
    exit 1
fi
...
echo "[ASTRA-sim] Running 76.1B GPT (1024 GPUs) with analytical congestion-aware backend..."
