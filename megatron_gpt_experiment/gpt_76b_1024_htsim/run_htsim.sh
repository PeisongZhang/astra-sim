#!/bin/bash
# megatron_gpt_76b_1024 on htsim, RoCE + GenericCustomTopology.  §11.6 gate.
set -o pipefail
set -x
...
if [ ! -x "${ASTRA_SIM}" ]; then
    echo "[htsim] Binary missing, building..."
    "${PROJECT_DIR:?}/build/astra_htsim/build.sh" || exit 1
fi

WORKLOAD_DIR_DEFAULT="${PROJECT_DIR}/../dnn_workload/megatron_gpt_76b/fused_standard_60_1_1792_2_2048_1f1b_v1_sgo1_ar1"
...
echo "[htsim] Running megatron_gpt_76b_1024 with --htsim-proto=${PROTO}..."
"${ASTRA_SIM:?}" \
