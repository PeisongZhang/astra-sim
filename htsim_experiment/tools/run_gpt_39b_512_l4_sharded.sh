#!/bin/bash
# Lighter acceptance variant: LAYER=4 (vs production 48). Same DP=32 TP=8 PP=2
# layout, batch=1536, micro_batch=4 → 12 microbatches. 12× less per-rank
# compute than the production LAYER=48 config — use it to validate the
# U2 sharded infrastructure at full 512-NPU scale within minutes of wall
# time.
#
# Analytical baseline: 1,445,483,841 cycles (captured 2026-04-22).
...
WORKLOAD_DIR="${WORKLOAD_DIR:-${ASTRA_DIR}/../dnn_workload/megatron_gpt_39b/fused_standard_4_1_1536_4_2048_1f1b_v1_sgo1_ar1}"
SHARD_WKLD_ROOT="${SHARD_WKLD_ROOT:-/tmp/shard_gpt39b_pp2_l4}"
OUT_DIR="${OUT_DIR:-${ASTRA_DIR}/htsim_experiment/gpt_39b_512_l4_sharded}"
QUEUE="${ASTRASIM_HTSIM_QUEUE_TYPE:-random}"
PROTO="${HTSIM_PROTO:-roce}"
ENDTIME="${ASTRASIM_HTSIM_ENDTIME_SEC:-30}"
ANALYTICAL_BASELINE_CYCLES=1445483841
...
echo "[gpt_39b_512_l4] launching 2 shards (QUEUE=${QUEUE}, ENDTIME=${ENDTIME}s simtime, LAYER=4)"
echo "[gpt_39b_512_l4] start: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
t0=$(date +%s)
...
wait ${pid0} ${pid1}
t1=$(date +%s)
echo "[gpt_39b_512_l4] end: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
...
if [ "${OK}" -ne 2 ]; then
    echo "[gpt_39b_512_l4] FAIL — not all shards reached 256/256"
    exit 1
fi
pass=$(awk "BEGIN{r=${MAX} / ${ANALYTICAL_BASELINE_CYCLES}; print (r >= 0.9 && r <= 1.5) ? 1 : 0}")
if [ "${pass}" -ne 1 ]; then
    echo "[gpt_39b_512_l4] FAIL — cycle ratio outside §11.6 window"
    exit 1
fi
echo "[gpt_39b_512_l4] PASS §11.6 cycle acceptance"
