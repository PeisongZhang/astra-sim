#!/bin/bash
# Secondary acceptance harness: micro_batch=4 variant of gpt_39b_512.
# Halves the per-rank iteration node count vs the default micro_batch=2
# workload, providing a faster acceptance signal when the main run is
# too slow in wall time.
#
# Analytical baseline for the micro=4 variant: 13,824,863,562 cycles
# (from megatron_gpt_experiment/gpt_39b_512/run_analytical.log.b4).
...
WORKLOAD_DIR="${WORKLOAD_DIR:-${ASTRA_DIR}/../dnn_workload/megatron_gpt_39b/fused_standard_48_1_1536_4_2048_1f1b_v1_sgo1_ar1}"
SHARD_WKLD_ROOT="${SHARD_WKLD_ROOT:-/tmp/shard_gpt39b_pp2_b4}"
OUT_DIR="${OUT_DIR:-${ASTRA_DIR}/htsim_experiment/gpt_39b_512_b4_sharded}"
QUEUE="${ASTRASIM_HTSIM_QUEUE_TYPE:-random}"
PROTO="${HTSIM_PROTO:-roce}"
ENDTIME="${ASTRASIM_HTSIM_ENDTIME_SEC:-300}"
ANALYTICAL_BASELINE_CYCLES=13824863562
...
if [ ! -f "${SHARD_WKLD_ROOT}/pp_shard_1/workload.255.et" ]; then
    echo "[gpt_39b_512_b4] splitting workload → ${SHARD_WKLD_ROOT}"
...
echo "[gpt_39b_512_b4] launching 2 shards (QUEUE=${QUEUE}, ENDTIME=${ENDTIME}s simtime, micro_batch=4)"
echo "[gpt_39b_512_b4] start: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
t0=$(date +%s)
...
wait ${pid0} ${pid1}
t1=$(date +%s)
echo "[gpt_39b_512_b4] end: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
...
if [ "${OK}" -ne 2 ]; then
    echo "[gpt_39b_512_b4] FAIL — not all shards reached 256/256"
    exit 1
fi
pass=$(awk "BEGIN{r=${MAX} / ${ANALYTICAL_BASELINE_CYCLES}; print (r >= 0.9 && r <= 1.5) ? 1 : 0}")
if [ "${pass}" -ne 1 ]; then
    echo "[gpt_39b_512_b4] FAIL — cycle ratio outside §11.6 window"
    exit 1
fi
echo "[gpt_39b_512_b4] PASS §11.6 cycle acceptance"
