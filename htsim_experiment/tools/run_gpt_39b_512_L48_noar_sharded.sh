...
WORKLOAD_DIR="${WORKLOAD_DIR:-${ASTRA_DIR}/../dnn_workload/megatron_gpt_39b/fused_standard_48_1_256_2_2048_1f1b_v1_sgo1_ar0}"
SHARD_WKLD_ROOT="${SHARD_WKLD_ROOT:-/tmp/shard_512_L48B256_noar}"
OUT_DIR="${OUT_DIR:-${ASTRA_DIR}/htsim_experiment/gpt_39b_512_L48_noar_sharded}"
QUEUE="${ASTRASIM_HTSIM_QUEUE_TYPE:-lossless}"
PROTO="${HTSIM_PROTO:-roce}"
ENDTIME="${ASTRASIM_HTSIM_ENDTIME_SEC:-300}"
ANALYTICAL_BASELINE_CYCLES=2917809088

if [ ! -f "${WORKLOAD_DIR}/workload.0.et" ]; then
    echo "[gpt_39b_512_L48_noar] Generating workload (LAYER=48 BATCH=256)..."
    (cd "${ASTRA_DIR}/../dnn_workload/megatron_gpt_39b" && \
        LAYER=48 DP=32 TP=8 PP=2 BATCH=256 MICROBATCH=2 ACTIVATION_RECOMPUTE=0 bash megatron_gpt_39b.sh) \
...
echo "[gpt_39b_512_L48_noar] launching 2 shards (L48, ~55min expected)..."
echo "[gpt_39b_512_L48_noar] start: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
t0=$(date +%s)
...
wait ${pid0} ${pid1}
t1=$(date +%s)
echo "[gpt_39b_512_L48_noar] end: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
...
if [ "${OK}" -ne 2 ]; then
    echo "[gpt_39b_512_L48_noar] FAIL — not all shards reached 256/256"
    exit 1
fi
pass=$(awk "BEGIN{r=${MAX} / ${ANALYTICAL_BASELINE_CYCLES}; print (r >= 0.9 && r <= 1.5) ? 1 : 0}")
if [ "${pass}" -ne 1 ]; then
    echo "[gpt_39b_512_L48_noar] FAIL — cycle ratio outside §11.6 window"
    exit 1
fi
echo "[gpt_39b_512_L48_noar] PASS §11.6 cycle acceptance (PRODUCTION LAYER=48)"
