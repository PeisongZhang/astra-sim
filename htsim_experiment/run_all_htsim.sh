...
EXPS=(
    ...
    "llama_experiment/in_dc_htsim"
    "llama_experiment/in_dc_dp_htsim"
    "llama_experiment/inter_dc_htsim"
    "llama_experiment/inter_dc_dp_htsim"
    "llama_experiment/inter_dc_dp_localsgd_htsim"
    "llama_experiment/inter_dc_mesh_htsim"
    "llama_experiment/inter_dc_ocs_mesh_htsim"
    "llama_experiment/inter_dc_ocs_ring_htsim"
    ...
    "llama3_70b_experiment/in_dc_htsim"
    "llama3_70b_experiment/inter_dc_dp_htsim"
    "llama3_70b_experiment/inter_dc_dp_localsgd_htsim"
    "llama3_70b_experiment/inter_dc_pp_htsim"
)
...
for exp in "${EXPS[@]}"; do
    runner="${PROJECT_DIR}/${exp}/run_htsim.sh"
    if [ ! -x "${runner}" ]; then
        echo "[run_all] skipping ${exp} (no run_htsim.sh)"
        echo "${exp},missing,,," >> "${REPORT}"
        continue
    fi
    t0=$(date +%s)
    echo "[run_all] running ${exp}..."
    ...
    base="${exp%_htsim}"
    for baselog in "${PROJECT_DIR}/${base}/run_analytical.log" "${PROJECT_DIR}/${base}/run.log"; do
        if [ -f "${baselog}" ]; then
